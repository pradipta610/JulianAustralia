...
@section('title', 'Edit Super User Admin | Julian Photography')
@section('content')
    <div class="w-[90%] md:w-[80%] mx-auto my-10 ">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg  bg-white dark:bg-gray-900 p-10">
            <form action="{{ route('superadminList.update', $data->id) }}" method="post">
                @csrf
                @method('patch')
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <input type="text" name="username" id="floating_name"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " value="{{ old('username', $data->username) }}" required />
                    <label for="floating_name"
