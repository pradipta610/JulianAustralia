...
@section('content')
    <div class="w-[90%] md:w-[80%] mx-auto my-10 ">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg  bg-white dark:bg-gray-900 p-10">
            <form action="{{ route('eventGallery.update', $data->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="relative z-0 w-full mb-6 group">
                    ...
                    @if ($data->image)
                        <img id="image_preview" src="{{ url('/storage/images/event/' . $data->image) }}" alt="Current Image" class="md:max-w-xs w-full">
                    @else
